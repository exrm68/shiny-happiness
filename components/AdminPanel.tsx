...
const AdminPanel: React.FC<AdminPanelProps> = ({ onClose }) => {
  ...
  const [comingSoonSeasons, setComingSoonSeasons] = useState<number[]>([]); // Lock entire seasons
  const [comingSoonSeasonInput, setComingSoonSeasonInput] = useState(''); // Input for adding seasons
  ...
  const handlePublish = async () => {
    ...
    try {
      const movieData: any = {
        ...
        ...(comingSoonSeasons.length > 0 && { comingSoonSeasons }),
      };
  ...
  const handleEdit = (movie: Movie) => {
    ...
    setComingSoonSeasons(movie.comingSoonSeasons || []);
  ...
  const resetForm = () => {
    ...
    setIsUpcoming(false);
    setReleaseDate('');
    setComingSoonSeasons([]);
    setComingSoonSeasonInput('');
  };
                ...
                {contentType === 'series' && (
                  <div className="bg-gray-900 rounded-lg p-4 space-y-4">
                    ...
                    <div className="bg-gradient-to-r from-yellow-900/20 to-orange-900/20 border border-yellow-500/30 rounded-lg p-3">
                      <div className="text-sm text-yellow-300 font-semibold mb-2">🔒 Lock Entire Season (Coming Soon)</div>
                      <div className="flex gap-2">
                        <input
                          type="number"
                          value={comingSoonSeasonInput}
                          onChange={(e) => setComingSoonSeasonInput(e.target.value)}
                          placeholder="Season number (e.g., 2)"
                          className="flex-1 px-3 py-2 bg-gray-800 border border-yellow-500/30 rounded text-white text-sm"
                        />
                        <button
                          type="button"
                          onClick={() => {
                            const seasonNum = parseInt(comingSoonSeasonInput);
                            if (seasonNum && !comingSoonSeasons.includes(seasonNum)) {
                              setComingSoonSeasons([...comingSoonSeasons, seasonNum]);
                              setComingSoonSeasonInput('');
                            }
                          }}
                      ...
                      {comingSoonSeasons.length > 0 && (
                        <div className="flex gap-2 mt-2 flex-wrap">
                          {comingSoonSeasons.map(s => (
                            <div key={s} className="flex items-center gap-1 bg-yellow-500/20 px-2 py-1 rounded border border-yellow-500/30">
                              <span className="text-xs text-yellow-300 font-semibold">Season {s} 🔒</span>
                              <button
                                type="button"
                                onClick={() => setComingSoonSeasons(comingSoonSeasons.filter(season => season !== s))}
                      ...
                      <p className="text-xs text-gray-500 mt-2">পুরা season lock করলে সেই season এর সব episodes "Coming Soon" হবে</p>
                    </div>
